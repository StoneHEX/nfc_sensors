...
{
uint8_t	rreg;
	HAL_I2C_Mem_Read(&SENSORS_I2C_BUS, LPS22DF_ADDR, (uint16_t )address, I2C_MEMADD_SIZE_8BIT,	&rreg, 1, NFC_SENSORS_I2C_TIMEOUT);
	return rreg;
}

static uint8_t	write_lps22df_reg(uint8_t address,uint8_t data)
{
	return HAL_I2C_Mem_Write(&SENSORS_I2C_BUS, LPS22DF_ADDR, (uint16_t )address, I2C_MEMADD_SIZE_8BIT, &data, 1, NFC_SENSORS_I2C_TIMEOUT);
}
...
{
	return HAL_I2C_Mem_Read(&SENSORS_I2C_BUS, LPS22DF_ADDR, (uint16_t )LPS22DF_PRESS_OUT_XL, I2C_MEMADD_SIZE_8BIT, pressure_p_ptr, PRESSURE_P_LEN, NFC_SENSORS_I2C_TIMEOUT);
}

uint8_t LPS22DF_Read_T_Data(uint8_t *pressure_t_ptr)
{
	return	HAL_I2C_Mem_Read(&SENSORS_I2C_BUS, LPS22DF_ADDR, (uint16_t )LPS22DF_TEMP_OUT_L, I2C_MEMADD_SIZE_8BIT, pressure_t_ptr, PRESSURE_T_LEN, NFC_SENSORS_I2C_TIMEOUT);
}
