...
{
uint8_t	rreg;
	HAL_I2C_Mem_Read(&SENSORS_I2C_BUS, STTS22_ADDR, (uint16_t )address, I2C_MEMADD_SIZE_8BIT,	&rreg, 1, NFC_SENSORS_I2C_TIMEOUT);
	return rreg;
}

static uint8_t	write_stts22h_reg(uint8_t address,uint8_t data)
{
	return HAL_I2C_Mem_Write(&SENSORS_I2C_BUS, STTS22_ADDR, (uint16_t )address, I2C_MEMADD_SIZE_8BIT, &data, 1, NFC_SENSORS_I2C_TIMEOUT);
}
...
{
	return	HAL_I2C_Mem_Read(&SENSORS_I2C_BUS, STTS22_ADDR, (uint16_t )STTS22H_TEMP_L_OUT, I2C_MEMADD_SIZE_8BIT, temperature_ptr, TEMPERATURE_LEN, NFC_SENSORS_I2C_TIMEOUT);
}
